...
struct CountryCode: Identifiable, Hashable {
    /// ISO 3166-1 alpha-2 region code (e.g. "UA").
    ...
    /// All ISO 3166-1 countries with a known calling code, sorted by name.
    static let all: [CountryCode] = buildAll()
    ...
    /// Longest-prefix match used to live-resolve the country while the user
    /// types a calling code (e.g. "+1" → 🇺🇸, "+1268" → 🇦🇬).
    static func match(prefix text: String) -> CountryCode? {
        let normalized = text.hasPrefix("+") ? text : "+" + text
        guard normalized.count > 1 else { return nil }
        return all
            .filter { normalized.hasPrefix($0.callingCode) }
            .max { $0.callingCode.count < $1.callingCode.count }
    }

    static func country(regionCode: String) -> CountryCode? {
        all.first { $0.regionCode == regionCode }
    }

    private static func buildAll() -> [CountryCode] {
        let locale = Locale(identifier: "en")
        return Locale.Region.isoRegions
            .map(\.identifier)
            .filter { $0.count == 2 }
            .compactMap { region in
                guard let code = dialCodes[region],
                      let name = locale.localizedString(forRegionCode: region)
                else { return nil }
                return CountryCode(
                    regionCode: region,
                    callingCode: code,
                    flag: flag(for: region),
                    name: name
                )
            }
    ...
    private static func flag(for regionCode: String) -> String {
    ...
    // ISO 3166-1 alpha-2 → E.164 calling code. NANP members carry their
    // full destination code (+1268, +1441, …) so live prefix resolution can
    // tell them apart from the United States (+1).
    private static let dialCodes: [String: String] = [
        ...
        "PM": "+508", "PN": "+64", "PR": "+1787", "PS": "+970", "PT": "+351",
